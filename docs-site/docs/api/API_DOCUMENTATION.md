...
Most resource endpoints are scoped under a workspace. The general pattern is:

```
/api/v1/workspaces/{workspaceId}/links
/api/v1/workspaces/{workspaceId}/domains
/api/v1/workspaces/{workspaceId}/analytics
/api/v1/workspaces/{workspaceId}/bio-pages
```

Auth and workspace listing endpoints are not workspace-scoped:
...
```json
{
  "success": true,
  "data": {
    "access_token": "v4.local.encrypted-paseto-token...",
...
> **Note**: Linkrift uses PASETO V4 Local (symmetric) tokens, not V4 Public. Tokens are encrypted, not signed.
...
```bash
curl https://api.linkrift.io/api/v1/workspaces \
  -H "Authorization: Bearer v4.local.encrypted-paseto-token..."
```

#### Token Refresh

```bash
curl -X POST https://api.linkrift.io/api/v1/auth/refresh \
  -H "Content-Type: application/json" \
  -d '{
...
```http
POST /v1/workspaces/{workspaceId}/links
```
...
```http
GET /v1/workspaces/{workspaceId}/links
```
...
```http
GET /v1/workspaces/{workspaceId}/links/{link_id}
```
...
```http
PUT /v1/workspaces/{workspaceId}/links/{link_id}
```
...
```http
DELETE /v1/workspaces/{workspaceId}/links/{link_id}
```
...
```http
POST /v1/workspaces/{workspaceId}/links/bulk
```
